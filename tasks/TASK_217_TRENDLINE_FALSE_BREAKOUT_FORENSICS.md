# Task 217: TRENDLINE_FALSE_BREAKOUT_FORENSICS

# Goal

Add diagnostics to classify Trendline Break outcomes into continuation, failed breakout, re-entry, immediate adverse, and delayed follow-through cases.

# Source Requirement

Owner requested a comprehensive follow-up task batch after the pattern/indicator/risk review of `quant_backtest` master. This task is part of the remediation plan for pattern execution correctness, indicator timing clarity, risk-management realism, score calibration, reporting, and final documentation/ledger reconciliation.

Priority: **P1**

# Extracted Roles

- Owner role: Project owner / quant research lead.
- Supporting roles:
  - Quant researcher: validate economic assumptions, score calibration, and OOS diagnostics.
  - System trading architect: maintain action, risk, sizing, cost, and execution contracts.
  - Backtest verification engineer: preserve no-lookahead, fill correctness, intrabar policy, and deterministic tests.
  - Code reviewer: enforce scope, safety, and architecture boundaries.
- Forbidden roles:
- Live trading implementation unless the task explicitly says otherwise.
- Real exchange order execution.
- Secret/key management changes outside documented safety scope.
- Unrelated frontend/backend/database changes unless listed in Scope.

# Context

- Trendline breaks are vulnerable to false breakout on 1m data.
- The risk planner already defines a soft invalidation rule for close re-entering the broken trendline side.
- Research needs to know whether losses are due to false breaks, late entries, or stop placement.

# Scope

- quant_bitcoin/patterns/trendline_break.py
- quant_bitcoin/patterns/trendline_break_risk_exit.py
- quant_bitcoin/backtesting/timing_diagnostics.py
- quant_bitcoin/backtesting/risk_exit_audit.py
- tests/backtesting/
- tests/patterns/

# Out of Scope

- Real Binance order execution.
- Live trading enablement.
- API keys, credentials, or `.env` changes.
- Portfolio optimization or machine learning model training unless explicitly listed in Requirements.
- Broad UI redesign beyond the listed frontend/read-only display requirements.
- Database schema changes unless explicitly required by this task.
- Silent behavior changes outside the named files and contracts.

# Requirements

- Classify post-break path over configurable bars: immediate_follow_through, reentered_trendline, failed_breakout, chopped_inside, no_follow_through.
- Record bars_to_reentry, max_favorable_r_before_reentry, and realized_r_after_reentry.
- Group by touch_count, slope, break_distance_atr, volume_ratio, displacement_confirmed, and pivot confirmation delay.
- Expose diagnostics in summary metadata and optional frontend/report consumers.

# Status Tracking

## Before Implementation

- [x] Read `AGENTS.md`.
- [x] Read `STATUS.md`.
- [x] Read `BACKLOG.md`.
- [x] Read `PROJECT_HISTORY.md` only as needed for this task's historical context.
- [x] Confirm this task matches the current phase and step.
- [x] Confirm the current active task is recorded or should be updated.
- [x] Confirm parallel work is allowed before starting any parallel tasks.
- [x] Record assumptions, blockers, or unclear status items before coding.
- [x] Identify exact source files and tests touched by this task.
- [x] Confirm no live trading, real order execution, signed exchange request, or secret handling is introduced.

Assumptions / notes:
- Implement post-break classification as offline backtesting diagnostics over trendline actions plus supplied candles.
- Reuse existing trendline metadata (`trendline_value`, direction, break distance, volume, touch count, pivot metadata) and avoid changing detector signal semantics.
- Exact files expected: a focused backtesting diagnostics module, canonical runner metadata wiring, and focused tests under `tests/backtesting/`.
- No live trading, real exchange order execution, signed exchange requests, credentials, or `.env` behavior are in scope.

## After Implementation

- [x] Update `STATUS.md` if the phase, step, goal, active task, blocker, open question, or completion state changed.
- [x] Mark checklist items complete only when acceptance criteria and verification are satisfied.
- [x] Leave uncertain items open and document the uncertainty.
- [x] Append concise completion note to `PROJECT_HISTORY.md` if this task is completed.
- [x] Update `BACKLOG.md` if this task creates, completes, blocks, splits, or reprioritizes follow-up work.
- [x] Confirm the next step is accurate or explicitly left undecided.

Completion notes:
- Added `trendline_false_breakout_forensics_v1` diagnostics for direction-aware Trendline entry actions.
- Classified post-break paths as immediate follow-through, re-entered trendline, failed breakout, chopped inside, or no follow-through over a configurable lookahead window.
- Recorded bars-to-reentry, favorable R before reentry, realized R after reentry, and grouped outcomes by touch count, slope, break distance, volume, displacement confirmation, and pivot delay.
- Wired the diagnostics into canonical strategy runner output diagnostics and summary metadata.
- No live trading, real exchange order execution, signed exchange requests, credentials, or `.env` behavior were added.

# Acceptance Criteria

- False breakout fixture is classified correctly.
- Strong continuation fixture is classified correctly.
- Diagnostics are direction-aware.

# Required Tests

## Unit Tests

- Unit: bullish close back below trendline classified reentry/failed.
- Unit: bearish symmetric case.
- Unit: strong follow-through no reentry classified continuation.

## Integration Tests

- Integration: trendline strategy run includes false_breakout_diagnostics.

## Contract Tests

- Add contract tests for metadata schemas, no-lookahead behavior, CLI/API output, or compatibility where applicable.

## Safety Tests

- Confirm no live trading path, real exchange order endpoint, signed exchange request, API key handling, or `.env` mutation is introduced.
- Confirm strategy/backtest modules remain offline simulation/research modules.


# Side Effects / Risks

- Requires additional candle path storage/access in diagnostics.

# Review Checklist

- [x] Scope respected.
- [x] Requirement matched.
- [x] Role ownership respected.
- [x] Architecture boundaries respected.
- [x] Data contract respected where applicable.
- [x] No hardcoded secrets.
- [x] No real order execution unless explicitly requested by a future owner-approved live task.
- [x] No unnecessary abstractions.
- [x] No lookahead introduced.
- [x] Pattern/risk/indicator semantics are documented in metadata or docs.
- [x] Tests cover both success and failure/skip paths.

Self-review summary:
- Implemented only the assigned Trendline false-breakout forensic diagnostics scope.
- Touched only focused backtesting runner/diagnostic code and focused tests.
- Added no order execution, signed exchange request, secret handling, frontend, backend API, or database schema behavior.
- Verification passed with targeted and broad suites.

# Verification

Default:

```bash
pytest
```

Recommended targeted verification for this task:

```bash
pytest tests/patterns tests/risk tests/backtesting
pytest tests/strategies
git diff --check
```

Verification run:

```bash
pytest tests/backtesting/test_trendline_forensics.py
pytest tests/backtesting/test_trendline_forensics.py tests/backtesting/test_pattern_postgres_runner_cli.py tests/backtesting/test_timing_diagnostics.py tests/backtesting/test_risk_exit_audit.py
pytest tests/patterns tests/risk tests/backtesting
pytest tests/strategies
git diff --check
```

If frontend files are changed:

```bash
cd frontend && npm run build
```

If backend/API files are changed and dependencies are available:

```bash
pytest backend/tests
```

# Codex Self-Review Requirement

Before completion, run through `reviews/CODEX_SELF_REVIEW.md` and include the result in the final summary.

# PR Review Requirement

Use `reviews/REVIEW_CHECKLIST.md` and `docs/06_PR_REVIEW_PROCESS.md` before merge.

# Completion Summary Required

- files changed
- implementation summary
- tests added or updated
- tests run
- Codex self-review result
- known limitations
- recommended next task
