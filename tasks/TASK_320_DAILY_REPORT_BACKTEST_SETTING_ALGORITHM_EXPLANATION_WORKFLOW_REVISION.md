...
- [x] Confirm this is documentation/workflow work only, not strategy/backtest implementation.
...
- Implementation summary: updated reusable daily-report guidance so future `백테스트 설정` sections explain important entry/exit/indicator/cost-guard mechanics with plain Korean and concise explanatory pseudocode, including lookback-return and ATR examples plus no-lookahead/warm-up/fill-timing guidance.
...
- Codex self-review result: scope respected; no strategy/backtest code, indicator implementation, report artifact, DB record, live trading behavior, exchange endpoint, secret, or `.env` change was introduced.
- Known limitations: exact source-derived code snippets remain opt-in; regular reports should default to explanatory pseudocode unless a future task explicitly requires source-level citation.
- Recommended next task: execute Task 321 if not already completed in the same owner-assigned batch.
