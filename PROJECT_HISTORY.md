...
- 2026-05-23: Created Task 139 `GITIGNORE_ARTIFACT_CLEANUP_AND_PR_RESCOPE` from owner request to create a clean scoped PR branch and add ignore rules for remaining generated/local artifacts; no implementation executed in this step per no-task/no-execution workflow.
- 2026-05-23: Completed Task 139 `GITIGNORE_ARTIFACT_CLEANUP_AND_PR_RESCOPE`; added `.gitignore` rules for generated/local artifacts, created a clean scoped branch for Tasks 130-139, left unrelated tracked frontend changes unstaged, and prepared PR rescope verification.
- 2026-05-23: Created and format-checked Tasks 140-148 (`POSITION_SIZING_POLICY_CONTRACT`, `LONG_CASH_BOUNDED_ENTRY_EXECUTION`, `SHORT_BUYING_POWER_POLICY`, `SIMULATED_MARGIN_INITIAL_MARGIN_GUARD`, `ACCOUNT_STATE_VISIBILITY_FIELDS`, `CANONICAL_CLI_PERSISTENCE_WIRING_FOR_SIZING_MARGIN`, `BACKTEST_CASH_EQUITY_DISPLAY_AND_API_SEMANTICS`, `BACKTEST_ACCOUNTING_REFACTOR_AFTER_CASH_AND_MARGIN`, `BACKTEST_ACCOUNTING_DOCUMENTATION_CONSISTENCY`) against `tasks/TASK_TEMPLATE.md`; set Task 140 as the active implementation task.
- 2026-05-23: Completed Task 140 `POSITION_SIZING_POLICY_CONTRACT`; added explicit backtest position-sizing/margin policy models with fixed-quantity, cash-fraction, target-notional, insufficiency policy, validation, metadata, and backward-compatible `trade_quantity` behavior.
...
- 2026-05-23: Completed Task 142 `SHORT_BUYING_POWER_POLICY`; made default simulated shorts cash-bounded so oversized short exposure is resized or blocked instead of silently opening full notional.
- 2026-05-23: Completed Task 143 `SIMULATED_MARGIN_INITIAL_MARGIN_GUARD`; added opt-in backtest-only simulated-margin initial-margin checks with leverage metadata and insufficient-margin blocking.
- 2026-05-23: Completed Task 144 `ACCOUNT_STATE_VISIBILITY_FIELDS`; added additive account-state fields for free cash, margin used, locked short proceeds, available buying power, and cash semantics in executions/equity/persistence metadata.
- 2026-05-23: Completed Task 145 `CANONICAL_CLI_PERSISTENCE_WIRING_FOR_SIZING_MARGIN`; wired sizing and simulated-margin options through the canonical strategy CLI, JSON output, and persistence metadata.
- 2026-05-23: Completed Task 146 `BACKTEST_CASH_EQUITY_DISPLAY_AND_API_SEMANTICS`; updated API contract and frontend display/types to distinguish cash balance from free cash when account-state metadata is available.
...
- 2026-05-23: Completed Task 148 `BACKTEST_ACCOUNTING_DOCUMENTATION_CONSISTENCY`; reconciled README, architecture/API/dashboard docs, task checklists, and ledgers after the Tasks 140-147 implementation window.
