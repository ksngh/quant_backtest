...
- Created (2026-06-01): Task 303 `DAILY_REPORT_INTERPRETATION_STYLE_WORKFLOW_REVISION` captures owner feedback for future Korean daily-report tone and structure. It is a documentation/workflow task to update daily-report template, style, workflow, handoff, data, and image guidance so future reports avoid awkward wording, omit absent pattern/filter/artifact mentions by default, enrich representative trade descriptions with available context, and make the interpretation section synthesize experiment intent, observed result, likely causes, and next improvements. It has not been implemented yet and excludes backtests, tuning/search, strategy/code changes, candle backfill, DB mutation, image generation, frontend/backend/API changes, live trading behavior, exchange endpoints, secrets, and `.env` changes.
