...
class IndicatorCache:
    ...
    @classmethod
    def for_fvg(cls, candles: pd.DataFrame | list[dict[str, Any]], config: FairValueGapConfig | None = None) -> "IndicatorCache":
        fvg_config = config or FairValueGapConfig()
        normalized = _normalize_candles(candles, None)
        atr_rows = calculate_atr(normalized[["symbol", "timestamp", "high", "low", "close"]], fvg_config.atr_config)
        volume_rows = calculate_volume_ratio(normalized[["symbol", "timestamp", "volume"]], fvg_config.volume_ratio_config)
        ...
        return cls(candles=enriched, atr=atr_rows["atr"], volume_ratio=volume_rows["volume_ratio"], displacement_rows=displacement_rows)
